...
const nav = [
  { href: "/skilltree", label: "Início", Icon: IconHome, exact: true },
...
export default function Shell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (ready && !loggedIn && pathname !== "/skilltree/login") {
      router.replace("/skilltree/login");
    }
  }, [ready, loggedIn, pathname, router]);

  if (pathname === "/skilltree/login") {
    ...
    <div className="st-root">
      <header className="st-glass" style={{ position: "sticky", top: 0, zIndex: 40, borderLeft: "none", borderRight: "none", borderTop: "none" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", padding: "12px 16px", display: "flex", alignItems: "center", gap: 12, justifyContent: "space-between" }}>
          <Link href="/skilltree" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none", color: "inherit" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <Link href="/skilltree/profile" className="st-btn st-btn-ghost" style={{ padding: "3px 10px 3px 3px", borderRadius: 9999, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <button type="button" aria-label="Som" onClick={() => { const nm = !muted; setMuted(nm); setMutedState(nm); if (!nm) { resumeAudio(); sfx.click(); } }} className="st-btn st-btn-ghost" style={{ width: 36, height: 36, padding: 0 }}>
              <IconMute off={muted} />
            </button>
          </div>
        </div>
      ...
      <main style={{ maxWidth: 1080, margin: "0 auto", padding: "22px 16px 110px" }}>{children}</main>

      <nav className="st-glass" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 40, borderBottom: "none", borderLeft: "none", borderRight: "none" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", justifyContent: "space-around", padding: "8px 8px calc(8px + env(safe-area-inset-bottom))" }}>
          ...
          <Link href="/" style={{ flex: 1, maxWidth: 110, textDecoration: "none", color: "var(--st-muted)", display: "flex", flexDirection: "column", alignItems: "center", gap: 3, padding: "6px 0", fontSize: "0.68rem" }}>
            <IconExternal /> NEXUS
          </Link>
        </div>
      </nav>
